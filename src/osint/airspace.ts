    ...
    try {
      for (const zone of this.zones) {
        ...
        const entity = this.viewer.entities.add({
          ...
          label: {
            text: `${zone.country.toUpperCase()} AIRSPACE ${this.statusLabel(zone.status)}\n${zone.notam_id}\nFrom: ${startStr}`,
            font: 'bold 13px JetBrains Mono',
            fillColor: baseColor.withAlpha(0.95),
            outlineColor: Cesium.Color.BLACK,
            outlineWidth: 3,
            style: Cesium.LabelStyle.FILL_AND_OUTLINE,
            backgroundColor: Cesium.Color.fromCssColorString('#0a0a0f').withAlpha(0.75),
            showBackground: true,
            backgroundPadding: new Cesium.Cartesian2(10, 6),
            pixelOffset: new Cesium.Cartesian2(0, 0),
            scaleByDistance: new Cesium.NearFarScalar(5e5, 1.0, 2e7, 0.35),
